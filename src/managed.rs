...
use std::{collections::{HashMap, VecDeque}, ops::{Deref, DerefMut}, sync::{Mutex, RwLockReadGuard, RwLockWriteGuard}};
...
impl<'a,T> Default for ObjectRef<'a,T>{
    fn default() -> Self {
        Self {
            idx:0, generation:0, parent:None
        }
    }
}
...
impl<T> ObjectBuffer<T>{    
    pub fn new()->Self{
        ...
        Self { buffer:buf.into(), free_queue:Mutex::new(Vec::new())}
    } 
    ...
    pub fn allocate_object<'a>(&'a self, item:T)->Result<ObjectRef<'a, T>, T>{
        for i in 1..self.buffer.len(){
            ...
            tmp.value = Some(item);
            return Ok(
                ObjectRef { idx: i as u64, generation: tmp.generation, parent: Some(self)}
            );
        }
        Err(item)
    }
    ...
    pub fn get_object_mut<'a,'b>(&'a self, rf:&'b ObjectRef<'a, T>)->Option<ObjectWrite<'a,T>>{
        if let Some(p) = rf.parent{
            if p as *const _ != self as *const _{
                return None;
            }else{
  ...
  pub fn try_get_object<'a,'b>(&'a self, rf:&'b ObjectRef<'a, T>)->Option<ObjectRead<'a,T>>{
    ...
    pub fn try_get_object_mut<'a,'b>(&'a self, rf:&'b ObjectRef<'a, T>)->Option<ObjectWrite<'a,T>>{
        if let Some(p) = rf.parent{
            if p as *const _ != self as *const _{
                return None;
            }else{
    ...
    pub fn run_free_queue(&self){
        ...
        for i in guard.iter(){
            let rf = ObjectRef{
                idx:i.0, generation:i.1, parent:Some(self)
            };
...
impl<T:Send+Sync> ObjectBuffer<T>{ 
    pub fn for_each_par(&self, to_run:impl Fn(ObjectRef<T>, &T)+Sync){
        ...
        (0..l).par_bridge().for_each(|i| {
            ...
            let rf = ObjectRef{idx:i as u64, generation:genr, parent:Some(self)};
    ...
    pub fn for_each_par_mut_no_block(&self, to_run:impl Fn(ObjectRef<T>, &mut T)+Sync){
        ...
        (0..l).par_bridge().for_each(|i| {
            ...
            let mut g = ObjectWrite{guard:lck};
            to_run(ObjectRef{
                generation:genr, idx:i as u64, parent:Some(&self)
            },&mut*g);
        });
    }
}
...
impl<T:Send+Sync+Clone> ObjectBuffer<T>{
    pub fn update(&self, to_run:impl Fn(ObjectRef<T>,&mut T)+Sync){
        let l = self.buffer.len();
        let output:Vec<ObjectSlot<T>>= (0..l).par_bridge().map(|i| {
            let lck = match self.buffer[i].read(){
                Ok(p)=>{
                    p
                }
            ...
            if let Some(x) = output.value.as_mut(){
                to_run(ObjectRef { idx: i as u64, generation: output.generation, parent: Some(self)}, x);
            }
            output
        }).collect();
...
impl<'a,T> ObjectRef<'a,T>{
    ...
    pub const fn new_invalid()->Self{
        Self { idx: 0, generation: 0, parent: None}
    }
...
impl<'a,T> Iterator for ObjectIterator<'a,T>{
    ...
    fn next(&mut self)->Option<Self::Item>{
        while self.idx<self.parent.buffer.len() as u64{
            let guard = match
                self.parent.buffer[self.idx as usize].read(){
                    Ok(p)=>p, 
                    Err(p)=>p.into_inner()
               };
            self.idx += 1;
...
impl<'a,T> Iterator for ObjectIteratorNonBlocking<'a,T>{
    ...
    fn next(&mut self)->Option<Self::Item>{
        while self.idx<self.parent.buffer.len() as u64{
            let guard = match
                self.parent.buffer[self.idx as usize].try_read(){
                    Ok(p)=>p, 
                    Err(p)=>{
                        match p{
                            std::sync::TryLockError::Poisoned(p)=>{
...
impl<'a,T> Iterator for ObjectIteratorMut<'a,T>{
    ...
    fn next(&mut self)->Option<Self::Item>{
        while self.idx<self.parent.buffer.len() as u64{
            let guard = match
                self.parent.buffer[self.idx as usize].write(){
                    Ok(p)=>p, 
                    Err(p)=>p.into_inner()
               };
            self.idx += 1;
...
impl<'a,T> Iterator for ObjectIteratorMutNonBlocking<'a,T>{
    ...
    fn next(&mut self)->Option<Self::Item>{
        while self.idx<self.parent.buffer.len() as u64{
            let guard = match
                self.parent.buffer[self.idx as usize].try_write(){
                    Ok(p)=>p, 
                    Err(p)=>{
                        match p{
                            std::sync::TryLockError::Poisoned(p)=>{
...
impl<T> EventBuffer<T>{
    ...
    pub fn pull(&self)->EventBufferPull<T>{
        EventBufferPull { inner: self.inner.clone() }
    }
}
...
impl<T:Send+Sync,U:Send+Sync> EventBuffer<Callback<T,U>>{
    pub fn drive_par(&self, context:&T)->Vec<U>{ 
        let out:Vec<Option<U>> = self.pull().par_bridge().map(|mut i|{
            match i.update(context){
                CallbackResult::Finished(x)=>{
                    return Some(x);
                }
        ...
        out.into_iter().filter(|i| i.is_some()).map(|i| i.unwrap()).collect()
    } 

    pub fn drive_par_discard(&self, context:&T){
        self.pull().par_bridge().for_each(|mut i|{
            match i.update(context){
                CallbackResult::Finished(x)=>{
                    _ = x;
                }
...
impl<T:Serialize+DeserializeOwned> TObjectBuffer for ObjectBuffer<T>{
    fn delete_object(&self, at:u64, generation:u64) {
        self.deallocate_object(&ObjectRef { idx: at, generation, parent: Some(self) });    
    } 
